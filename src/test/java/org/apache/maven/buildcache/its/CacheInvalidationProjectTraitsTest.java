 ...
 *   <li>2.3 Dep Mgmt Edge Cases: BOM version (P05), BOM import order (P06), exclusion, optional flag, classifier dep (P06)</li>
 ...
 *   <li>2.6 Local/Inherited Parent: property change in local parent POM cascades to children (P02)</li>
...
class CacheInvalidationProjectTraitsTest {
    ...
    /**
     * TC: BomImportOrderChangeInvalidates (P06)
     *
     * <p>Maven resolves BOM-managed dependency versions using first-wins mediation: when two imported
     * BOMs both manage the same artifact, the one listed first in {@code <dependencyManagement>} wins.
     * Swapping the BOM import order therefore changes the resolved version of the shared dependency
     * and must produce a cache miss.
     *
     * <p>P06 ships two BOMs with <em>different</em> Maven coordinates (avoiding the Maven 4
     * uniqueness restriction on {@code groupId:artifactId:type:classifier}):
     * <ul>
     *   <li><b>BOM-A</b>: {@code com.google.guava:guava-bom:31.1-jre} — manages guava at 31.1-jre</li>
     *   <li><b>BOM-B</b>: {@code org.apache.maven.caching.test.p06:bom-b:1.0-SNAPSHOT} (a reactor
     *       module) — manages guava at 30.1.1-jre</li>
     * </ul>
     *
     * <p>In the reference project BOM-A is listed first, so guava resolves to {@code 31.1-jre}.
     * The test swaps the two imports; BOM-B is now first and guava resolves to {@code 30.1.1-jre}.
     * The effective POM changes → Build 2 must be a cache miss.
     *
     * <p>Works with both Maven 3 (3.3.1+) and Maven 4. Maven's workspace reader resolves the
     * reactor BOM module's POM directly from the source tree without requiring a prior install.
     */
    @Test
    void bomImportOrderChangeInvalidates() throws Exception {
        Path p06 = Paths.get("src/test/projects/reference-test-projects/p06-dep-edge-cases")
                .toAbsolutePath();
        Verifier verifier = ReferenceProjectBootstrap.prepareProject(p06, "bomImportOrderChangeInvalidates");
        verifier.setAutoclean(false);
        // Skip the enforcer: after the swap, guava downgrades from 31.1 to 30.1.1 which may
        // trigger requireUpperBoundDeps. The test is about BOM ordering, not enforcer behaviour.
        verifier.addCliOption("-Denforcer.skip=true");

        // Build 1 — BOM-A (guava-bom:31.1-jre) listed first; guava resolves to 31.1-jre; saved
        verifier.setLogFileName("../log-1.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_SAVED);

        // Swap the two BOM imports so BOM-B (bom-b, guava:30.1.1-jre) is now first and wins
        Path rootPom = Paths.get(verifier.getBasedir(), "pom.xml");
        replaceInFile(
                rootPom,
                "            <!-- BOM-A \u2014 com.google.guava:guava-bom:31.1-jre; listed first \u2192 wins by default -->\n"
                        + "            <dependency>\n"
                        ...
                        + "                <scope>import</scope>\n"
                        + "            </dependency>\n"
                        + "            <!-- BOM-B \u2014 reactor module bom-b; manages guava:30.1.1-jre; listed second \u2192 loses by default -->\n"
                        + "            <dependency>\n"
                        + "                <groupId>org.apache.maven.caching.test.p06</groupId>\n"
                        ...
                        + "                <scope>import</scope>\n"
                        + "            </dependency>",
                "            <!-- BOM-B \u2014 reactor module bom-b; manages guava:30.1.1-jre; now listed first \u2192 wins -->\n"
                        + "            <dependency>\n"
                        + "                <groupId>org.apache.maven.caching.test.p06</groupId>\n"
                        + "                <artifactId>bom-b</artifactId>\n"
                        + "                <version>${project.version}</version>\n"
                        ...
                        + "                <scope>import</scope>\n"
                        + "            </dependency>\n"
                        + "            <!-- BOM-A \u2014 com.google.guava:guava-bom:31.1-jre; now listed second \u2192 loses -->\n"
                        + "            <dependency>\n"
                        + "                <groupId>com.google.guava</groupId>\n"
                        + "                <artifactId>guava-bom</artifactId>\n"
                        + "                <version>31.1-jre</version>\n"
        ...
        // Build 2 — BOM-B (30.1.1-jre) now wins; effective guava version changes 31.1→30.1.1;
        // effective POM differs from Build 1 → cache miss
        verifier.setLogFileName("../log-2.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_MISS);
    ...
    // -----------------------------------------------------------------------
    // 2.1 System-scope JAR content change
    // -----------------------------------------------------------------------

    /**
     * Verifies that a content change to a system-scope JAR invalidates the cache (CINV-1.1).
     *
     * <p>P01 declares {@code lib/placeholder.jar} as a system-scope dependency.  The cache
     * extension hashes the file at the declared {@code systemPath} only for
     * <em>SNAPSHOT-versioned</em> system-scope dependencies (release-versioned ones are treated
     * as immutable and skipped).  The test therefore patches the dependency version to
     * {@code 1.0-SNAPSHOT} before Build 1, then replaces the JAR bytes and verifies a cache
     * miss on Build 2.
     */
    @Test
    void systemScopeJarContentChangeInvalidates() throws Exception {
        Path p01 = Paths.get("src/test/projects/reference-test-projects/p01-superpom-minimal")
                .toAbsolutePath();
        Verifier verifier = ReferenceProjectBootstrap.prepareProject(p01, "systemScopeJarContentChangeInvalidates");
        verifier.setAutoclean(false);

        // Patch: change the system-scope dep version from 1.0 (release → immutable → skipped)
        // to 1.0-SNAPSHOT so the cache extension hashes the local file on every build.
        Path pom = Paths.get(verifier.getBasedir(), "pom.xml");
        replaceInFile(
                pom,
                "            <version>1.0</version>\n"
                        + "            <scope>system</scope>\n"
                        + "            <systemPath>${project.basedir}/lib/placeholder.jar</systemPath>",
                "            <version>1.0-SNAPSHOT</version>\n"
                        + "            <scope>system</scope>\n"
                        + "            <systemPath>${project.basedir}/lib/placeholder.jar</systemPath>");

        // Build 1 — cold cache, original placeholder.jar → saved
        verifier.setLogFileName("../log-1.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_SAVED);

        // Replace placeholder.jar with a valid but different empty ZIP archive.
        // The original is a 22-byte empty ZIP (EOCD record, no comment).
        // Writing 30 bytes — same EOCD structure with an 8-byte "MODIFIED" comment —
        // changes the file hash while keeping the archive structurally valid.
        Path jarPath = Paths.get(verifier.getBasedir(), "lib", "placeholder.jar");
        Files.write(jarPath, new byte[] {
            0x50,
            0x4B,
            0x05,
            ...
            0x00,
            0x00,
            0x00, // disk numbers (this disk / central-dir start disk)
            0x00,
            0x00,
            0x00,
            0x00, // entry counts (on this disk / total)
            0x00,
            0x00,
            0x00,
            ...
            0x49,
            0x45,
            0x44 // comment = "MODIFIED"
        });

        // Build 2 — JAR hash changed → cache miss
        verifier.setLogFileName("../log-2.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_MISS);
    ...
    // -----------------------------------------------------------------------
    // 2.3 BOM managed-but-undeclared dependency version change (no over-invalidation)
    // -----------------------------------------------------------------------

    /**
     * Verifies that changing a BOM version for a dependency that is <em>managed</em> but never
     * <em>declared</em> by any module does NOT invalidate the cache (CINV-2.3
     * BomVersionChangeUnusedDepNoMiss, negative test).
     *
     * <p>The cache fingerprint is derived from each module's <strong>declared</strong>
     * dependencies (with their resolved versions), not from the raw
     * {@code <dependencyManagement>} block.  A BOM entry that is managed but never
     * referenced by a {@code <dependency>} in any module has no effect on any module's
     * resolved dependency list, so changing its version must leave the fingerprint
     * unchanged.
     *
     * <h2>Test scenario</h2>
     * <p>Uses P06 ({@code p06-dep-edge-cases}).  The reactor BOM {@code bom-b} is patched
     * (before Build 1) to also manage {@code commons-lang:commons-lang:2.6}.  No module in
     * P06 declares {@code commons-lang} as a dependency.
     *
     * <ol>
     *   <li>Build 1: P06 built with bom-b managing {@code commons-lang:2.6} → all modules
     *       saved.</li>
     *   <li>Mutate: bom-b's managed {@code commons-lang} version changed from {@code 2.6} to
     *       {@code 2.4} (unused by all modules).</li>
     *   <li>Build 2: <em>no</em> module misses.  bom-b is a POM-only module with no declared
     *       dependencies (only {@code <dependencyManagement>}), and {@code <dependencyManagement>}
     *       is not included in the normalized model.  Therefore bom-b's fingerprint is also
     *       unchanged → all modules hit the cache.</li>
     * </ol>
     */
    @Test
    void bomManagedUnusedDepVersionChangeDoesNotInvalidate() throws Exception {
        Path p06 = Paths.get("src/test/projects/reference-test-projects/p06-dep-edge-cases")
                .toAbsolutePath();
        Verifier verifier =
                ReferenceProjectBootstrap.prepareProject(p06, "bomManagedUnusedDepVersionChangeDoesNotInvalidate");
        verifier.setAutoclean(false);

        // Pre-patch bom-b to also manage commons-lang:2.6.
        // No module in P06 declares commons-lang, so this entry is purely "managed".
        Path bombPom = Paths.get(verifier.getBasedir(), "bom-b", "pom.xml");
        replaceInFile(
                bombPom,
        ...
        // Build 1 — bom-b manages commons-lang:2.6 (unused); cold cache → all saved
        verifier.setLogFileName("../log-1.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_SAVED);

        // Change the managed commons-lang version from 2.6 → 2.4 (still unused).
        replaceInFile(bombPom, "<version>2.6</version>", "<version>2.4</version>");

        // Build 2:
        //   - all modules: CACHE_HIT — the managed commons-lang version is not in any module's
        //     declared dependency list, so no fingerprint changes.  bom-b itself also hits
        //     because <dependencyManagement> is not part of the normalized model.
        verifier.setLogFileName("../log-2.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
    ...
    // -----------------------------------------------------------------------
    // 2.6 Inherited / local-parent property change
    // -----------------------------------------------------------------------

    /**
     * Verifies that a property change in the local parent POM invalidates the cache for all
     * inheriting child modules (CINV-2.2 InheritedParentPropertyChange, CINV-2.5
     * LocalParentPropertyChange).
     *
     * <p>The cache fingerprint is derived from each module's <em>declared</em> dependency list
     * (with resolved versions).  {@code <properties>} is not directly hashed, but a property
     * that drives a managed dependency version propagates into the resolved version visible in
     * each child's declared dep list.
     *
     * <p>P02's root POM defines {@code junit.version=4.13.2}.  Its {@code <dependencyManagement>}
     * manages {@code junit:junit} at {@code ${junit.version}}.  All three child modules declare
     * {@code junit:junit} without a version, so their resolved junit version comes from this
     * property.  Bumping {@code junit.version} from {@code 4.13.2} to {@code 4.12} changes
     * the resolved declared dep in every child module → fingerprint changes → cache miss.
     */
    @Test
    void parentPropertyChangeInvalidates() throws Exception {
        Path p02 = Paths.get("src/test/projects/reference-test-projects/p02-local-parent-inherit")
                .toAbsolutePath();
        Verifier verifier = ReferenceProjectBootstrap.prepareProject(p02, "parentPropertyChangeInvalidates");
        verifier.setAutoclean(false);

        // Build 1 — junit.version=4.13.2, cold cache → saved
        verifier.setLogFileName("../log-1.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_SAVED);

        // Bump the junit.version property in the root (parent) POM from 4.13.2 → 4.12.
        // The managed junit version changes → resolved junit version in every child module's
        // declared dep list changes → all child module fingerprints change → all miss.
        Path rootPom = Paths.get(verifier.getBasedir(), "pom.xml");
        replaceInFile(rootPom, "<junit.version>4.13.2</junit.version>", "<junit.version>4.12</junit.version>");

        // Build 2 — all child modules see changed resolved junit version → cache miss
        verifier.setLogFileName("../log-2.txt");
        verifier.executeGoal("verify");
        verifier.verifyErrorFreeLog();
        verifier.verifyTextInLog(CACHE_MISS);
