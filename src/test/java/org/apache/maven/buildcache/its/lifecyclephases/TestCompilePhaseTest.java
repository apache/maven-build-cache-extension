...
/**
 * Verifies that {@code mvn test-compile} saves a cache entry and that a second identical
 * invocation restores from cache rather than re-compiling test sources (test 2.3).
 * Surefire execution is skipped in the project pom so only test-compile is exercised.
 */
@IntegrationTest("src/test/projects/lifecycle-phases")
class TestCompilePhaseTest {
    ...
    @Test
    void testCompilePhaseIsCachedAndRestored(Verifier verifier) throws VerificationException {
        verifier.setAutoclean(false);

        // Build 1 — test-compile; cache entry saved at test-compile level
