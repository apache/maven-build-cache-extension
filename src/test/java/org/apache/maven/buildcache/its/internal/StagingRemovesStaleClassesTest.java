 ...
 * </ol>
 *
 * <p>The stale-class staging mechanism operates at the output-artifact level and is entirely
 * independent of project structure. Additionally, multi-module project roots have no
 * {@code target/classes/} directory of their own (only child modules do), so those projects
 * would silently skip the stale-file injection and degrade to a plain cache-hit check already
 * covered by {@code CacheBaseBehaviorParametrizedTest#base01FirstBuildSavesSecondHits}.
 * A single-module project with a definite {@code target/classes/} is the correct fixture here.
...
class StagingRemovesStaleClassesTest {
    ...
    @Test
    void staleClassDoesNotAffectCacheHit() throws Exception {
        Verifier verifier = ReferenceProjectBootstrap.prepareProject(
                Paths.get("src/test/projects/reference-test-projects/p01-superpom-minimal"), "STALE");
