...
| `-Dmaven.build.cache.cacheSingleGoal=(true/false)`         | Cache goals invoked directly from the command line (e.g. `mvn compiler:compile`) when their default lifecycle phase is a real, post-clean phase. Goals with no default phase (e.g. `jetty:run`) and mixed lifecycle+CLI invocations are unaffected. (default is true) | Performance optimization for direct goal invocations                                            |
| `-Dmaven.build.cache.restoreForkedExecutions=(true/false)` | Restore cached results during a forked lifecycle triggered by a directly invoked goal via `@Execute(phase=...)` (e.g. the `test-compile` fork of `jetty:run`). The fork restores but never saves. Only effective with the singlethreaded/multithreaded builders. (default is true) | Avoid recompiling before running `jetty:run`, `spring-boot:run`, etc.                           |
