...
| F1.4 | Config path override                          | `-Dmaven.build.cache.configPath`            | P1          | ❌      | —                                                                          |
| F1.5 | Defaults-only mode (no XML file present)      | implicit                                    | P1          | ❌      | —                                                                          |
| F1.6 | Maven version guard (requires ≥ 3.9.0)        | `CacheConfigImpl.initialize()`              | P1          | ❌      | —                                                                          |
| F1.7 | XML config validation                         | `<validateXml>`                             | P2          | ❌      | —                                                                          |
| F1.8 | Malformed XML → descriptive startup error     | implicit                                    | P1          | ❌      | —                                                                          |
...
| F2.12 | Dependency version change → cache miss                | effective POM                                               | P0          | ❌      | —                                                            |
...
| F2.15 | Effective POM property exclusion per plugin           | `<plugin><effectivePom><excludeProperties>`                 | P1          | ❌      | —                                                            |
| F2.16 | Plugin dependency exclusion from fingerprint          | `<plugin><excludeDependencies>`                             | P2          | ❌      | —                                                            |
| F2.17 | `processPlugins` flag                                 | `maven.build.cache.processPlugins`                          | P1          | ❌      | —                                                            |
| F2.18 | Project-level glob override (POM property)            | `maven.build.cache.input.glob` in `<properties>`            | P1          | ❌      | —                                                            |
| F2.19 | Project-level additional include (POM property)       | `maven.build.cache.input.*` in `<properties>`               | P1          | ❌      | —                                                            |
| F2.20 | Project-level exclude (POM property)                  | `maven.build.cache.exclude.*` in `<properties>`             | P1          | ❌      | —                                                            |
| F2.21 | Profile activation changes effective POM → cache miss | `-P profile`                                                | P1          | ❌      | —                                                            |
...
| F3.2 | SHA-256                                       | `<hashAlgorithm>SHA-256`  | P2          | ❌      | —                  |
...
| F3.7 | Invalid algorithm → startup failure           | `<hashAlgorithm>BOGUS`    | P1          | ❌      | —                  |
...
| ID    | Feature                                    | XML / System Property                       | Criticality | Status | Test Reference                                             |
|-------|--------------------------------------------|---------------------------------------------|-------------|--------|------------------------------------------------------------|
| F4.1  | `runAlways` by plugin coordinates          | `<executionControl><runAlways><plugins>`    | P1          | ❌      | —                                                          |
| F4.2  | `runAlways` by goal name                   | `<executionControl><runAlways><goalsLists>` | P1          | ❌      | —                                                          |
| F4.3  | `runAlways` by execution ID                | `<executionControl><runAlways><executions>` | P1          | ❌      | —                                                          |
| F4.4  | `alwaysRunPlugins` CLI property            | `-Dmaven.build.cache.alwaysRunPlugins`      | P1          | ❌      | —                                                          |
| F4.5  | `ignoreMissing` — skip absent executions   | `<executionControl><ignoreMissing>`         | P1          | ❌      | —                                                          |
...
| F4.9  | Reconcile: `defaultValue` used when absent | `<property defaultValue="…">`               | P1          | ❌      | —                                                          |
| F4.10 | Reconcile: `logAll` flag on goal           | `<reconcile><plugin logAll="true">`         | P3          | ✅      | `pluginexecution/LogAllPropertiesTest`                     |
| F4.11 | Reconcile: `nolog` suppresses property     | `<property nolog="true">`                   | P3          | ❌      | —                                                          |
| F4.12 | `logAllProperties` global flag             | `<reconcile><logAllProperties>`             | P3          | ❌      | —                                                          |
...
| F4.15 | Missing cached execution → full rebuild    | `Build.getMissingExecutions()`              | P1          | ❌      | —                                                          |
...
| F5.4 | `restoreGeneratedSources=false`                 | `-Dmaven.build.cache.restoreGeneratedSources` | P1          | ❌      | —                                            |
| F5.5 | `restoreOnDiskArtifacts=false`                  | `-Dmaven.build.cache.restoreOnDiskArtifacts`  | P1          | ❌      | —                                            |
| F5.6 | Corrupted/truncated ZIP handled gracefully      | implicit                                      | P1          | ✅      | `failurerecovery/CorruptedZipCacheEntryTest` |
| F5.7 | NORMALIZED_VERSION → SNAPSHOT bump is cache hit | `calculateProjectVersionChecksum=false`       | P2          | ❌      | —                                            |
...
| F6.3 | `preservePermissions` — POSIX permissions        | `<attachedOutputs preservePermissions="…">` | P2          | ❌      | —                                                             |
| F6.4 | Output exclude patterns (regex)                  | `<output><exclude><patterns>`               | P1          | ❌      | —                                                             |
| F6.5 | `maxBuildsCached` LRU eviction                   | `<local><maxBuildsCached>`                  | P2          | ❌      | —                                                             |
| F6.6 | Custom local cache location                      | `-Dmaven.build.cache.location`              | P2          | ✅      | `BuildExtensionTest.skipSaving`                               |
| F6.7 | Stale artifacts in `target/` not leaked to cache | `stagePreExistingArtifacts` internal        | P2          | ❌      | —                                                             |
...
| F7.4 | `save.final` — prohibit overwrite            | `-Dmaven.build.cache.remote.save.final`                               | P2          | ❌      | —                    |
| F7.5 | Server authentication via settings.xml       | `<remote id="…">` / `-Dmaven.build.cache.remote.server.id`            | P1          | ✅      | `RemoteCacheDavTest` |
| F7.6 | Remote unavailable → graceful local fallback | implicit                                                              | P2          | ❌      | —                    |
| F7.7 | `failFast` on remote restore failure         | `-Dmaven.build.cache.failFast`                                        | P2          | ❌      | —                    |
| F7.8 | `baselineUrl` diff report                    | `-Dmaven.build.cache.baselineUrl`                                     | P3          | ❌      | —                    |
...
| F8.2 | Reactor cascade — upstream change → downstream miss | implicit                                        | P1          | ❌      | —                    |
| F8.3 | Partial reactor `-pl`                               | implicit `-pl`                                  | P1          | ❌      | —                    |
| F8.4 | Partial reactor `-pl -am` upstream restore          | implicit `-pl -am`                              | P1          | ❌      | —                    |
| F8.5 | Parallel builds `-T` — no corruption                | implicit `-T`                                   | P1          | ❌      | —                    |
| F8.6 | `scanProfiles` — profiles in multi-module key       | `<multiModule><scanProfiles>`                   | P2          | ❌      | —                    |
...
| F9.7 | `clean verify` → clean runs first, then cache           | implicit                                                  | P1          | ❌      | —                                                                      |
...
| F10.3 | `failFast` — abort on restore failure        | `-Dmaven.build.cache.failFast`       | P2          | ❌      | —                                             |
...
| F10.6 | `maxBuildsCached` LRU eviction               | `<local><maxBuildsCached>`           | P2          | ❌      | —                                             |
...
| F11.2 | XML file at custom path                | `-Dmaven.build.cache.configPath`       | P1          | ❌      | —                                                         |
...
| F11.5 | No XML file → built-in defaults active | implicit                               | P1          | ❌      | —                                                         |
| F11.6 | `alwaysRunPlugins` CLI comma-list      | `-Dmaven.build.cache.alwaysRunPlugins` | P1          | ❌      | —                                                         |
...
| ID    | Feature                                       | XML / System Property                                  | Criticality | Status | Test Reference |
|-------|-----------------------------------------------|--------------------------------------------------------|-------------|--------|----------------|
| F12.1 | `adjustMetaInf` — normalize MANIFEST.MF       | `<projectVersioning><adjustMetaInf>`                   | P2          | ❌      | —              |
| F12.2 | `calculateProjectVersionChecksum`             | `<projectVersioning><calculateProjectVersionChecksum>` | P2          | ❌      | —              |
| F12.3 | NORMALIZED_VERSION: SNAPSHOT bump → cache hit | implicit (default)                                     | P2          | ❌      | —              |
| F12.4 | CI-friendly `${revision}` version stability   | flatten-maven-plugin pattern                           | P1          | ❌      | —              |
...
| F13.2 | `logAllProperties` global flag                      | `<reconcile><logAllProperties>`       | P3          | ❌      | —                                      |
| F13.3 | `buildinfo.xml` FileHash debug                      | `<debugs><debug>FileHash</debug>`     | P3          | ❌      | —                                      |
| F13.4 | `buildinfo.xml` EffectivePom debug                  | `<debugs><debug>EffectivePom</debug>` | P3          | ❌      | —                                      |
| F13.5 | `build-cache-report.xml` generated per build        | implicit                              | P2          | ❌      | —                                      |
| F13.6 | `buildsdiff.xml` generated with baselineUrl         | `-Dmaven.build.cache.baselineUrl`     | P3          | ❌      | —                                      |
| F13.7 | CacheSource field (LOCAL/REMOTE/BUILD) in buildinfo | `Build.source`                        | P3          | ❌      | —                                      |
...
**Test class:** `CacheBaseBehaviorParametrizedTest` ❌ (not yet implemented)
...
|---------|------------------------------------------------------|------------------|---------|
| BASE-01 | First build completes; identical second build is hit | F1.1, F1.2, F5.1 | ❌ (new) |
| BASE-02 | Source file modification → cache miss                | F2.6             | ❌ (new) |
| BASE-03 | POM property value change → cache miss               | F2.9             | ❌ (new) |
| BASE-04 | `enabled=false` CLI → build runs, cache bypassed     | F1.3             | ❌ (new) |
| BASE-05 | `skipCache=true` → rebuild occurs; cache written     | F10.1            | ❌ (new) |
| BASE-06 | `skipSave=true` → cache hit; no new write            | F10.2            | ❌ (new) |
| BASE-07 | Compile→Package phase escalation → rebuild           | F9.5             | ❌ (new) |
...
| A-11 | Dependency version change → miss | F2.12            | **P01**                        | ❌ `DependencyVersionChangeInvalidatesCacheTest`  |
...
| B-02 | Project-level glob override via POM property     | F2.18     | **P02**                   | ❌ `PerProjectGlobOverrideTest`          |
| B-03 | Project-level include via POM property           | F2.19     | **P02**                   | ❌ `ProjectLevelIncludeTest`             |
| B-04 | `processPlugins=false` skips param introspection | F2.17     | **P01**                   | ❌ `ProcessPluginsDisabledTest`          |
| B-05 | Per-plugin dirScan limits scanned paths          | F2.13     | **P19**                   | ❌ `PerPluginDirScanTest`                |
| B-06 | Effective POM property exclusion per plugin      | F2.15     | **P19**                   | ❌ `EffectivePomExcludePropertyTest`     |
| B-07 | Profile activation changes effective POM → miss  | F2.21     | **P08**                   | ❌ `ProfileCliActivationInvalidatesTest` |
...
| C-07 | `clean verify` — clean then cached build | F9.7       | PARAM P01-P19              | ❌ `CleanVerifyTest`                            |
...
| D-07 | `runAlways` by plugin — plugin always runs   | F4.1         | **P19**           | ❌ `RunAlwaysPluginTest`                     |
| D-08 | `runAlways` by goal name                     | F4.2         | **P19**           | ❌ `RunAlwaysByGoalTest`                     |
| D-09 | `runAlways` by execution ID                  | F4.3         | **P19**           | ❌ `RunAlwaysByExecutionIdTest`              |
| D-10 | `alwaysRunPlugins` CLI property              | F4.4, F11.6  | **P01**           | ❌ `AlwaysRunPluginsCliTest`                 |
| D-11 | `ignoreMissing` — absent executions skipped  | F4.5         | **P19**           | ❌ `IgnoreMissingPluginTest`                 |
| D-12 | Reconcile `defaultValue` for absent property | F4.9         | **P19**           | ❌ `TrackedPropertyDefaultValueTest`         |
| D-13 | Missing cached execution → full rebuild      | F4.15        | **P19**           | ❌ `MissingExecutionTriggerRebuildTest`      |
...
| E-02 | `restoreGeneratedSources=false` skips sources     | F5.4             | **P01**                           | ❌ `RestoreGeneratedSourcesFalseTest` |
| E-03 | `restoreOnDiskArtifacts=false` skips target/      | F5.5             | **P01**                           | ❌ `RestoreOnDiskArtifactsFalseTest`  |
...
| ID   | Scenario                                        | Features    | Reference Project | Status                          |
|------|-------------------------------------------------|-------------|-------------------|---------------------------------|
| F-01 | Output exclude pattern filters from cache entry | F6.4        | **P17**           | ❌ `OutputExcludePatternTest`    |
| F-02 | `preservePermissions=true` restores POSIX bits  | F6.3        | **P01**           | ❌ `PermissionsPreservationTest` |
| F-03 | `maxBuildsCached` evicts oldest entries         | F6.5, F10.6 | **P01**           | ❌ `MaxLocalBuildsCachedTest`    |
...
| G-06 | `failFast=true` aborts on restore failure     | F7.7, F10.3 | **P01**           | ❌ `FailFastTest`                                                 |
...
| H-04 | configPath override to non-default location  | F11.2, F1.4 | **P01**                   | ❌ `CustomConfigPathTest`     |
| H-05 | No XML file → built-in defaults active       | F11.5, F1.5 | **P01** (no-config)       | ❌ `NoConfigFileDefaultsTest` |
| H-06 | Malformed XML → descriptive startup error    | F1.8        | **P01**                   | ❌ `InvalidConfigXmlTest`     |
...
| I-03 | Upstream module change → downstream miss          | F8.2       | **P02**                   | ❌ `UpstreamModuleChangeDownstreamMissTest` |
| I-04 | Partial reactor `-pl module`                      | F8.3       | **P10**                   | ❌ `MultiModulePartialBuildTest`            |
| I-05 | Partial reactor `-pl module -am` upstream restore | F8.4       | **P10**                   | ❌ `MultiModulePartialWithAmTest`           |
| I-06 | Parallel build `-T2` cache correctness            | F8.5       | **P11**                   | ❌ `ParallelBuildTest`                      |
...
| J-02 | Remote unavailable → local fallback  | F7.6            | legacy: `dav`     | ❌ `RemoteUnavailableFallbackTest` |
| J-03 | `save.final=true` — no overwrite     | F7.4            | legacy: `dav`     | ❌ `SaveFinalRemoteTest`           |
| J-04 | Baseline URL diff report             | F7.8, F13.6     | **P01** + remote  | ❌ `BaselineDiffTest`              |

### Group K: Hash Algorithms (P2)

| ID   | Scenario                            | Features | Reference Project | Status                       |
|------|-------------------------------------|----------|-------------------|------------------------------|
| K-01 | SHA-256 algorithm correct hit/miss  | F3.2     | PARAM P01-P19     | ❌ `HashAlgorithmSHA256Test`  |
| K-02 | Invalid algorithm → startup failure | F3.7     | **P01**           | ❌ `InvalidHashAlgorithmTest` |

### Group L: Project Versioning (P2)

| ID   | Scenario                                                 | Features    | Reference Project | Status                                      |
|------|----------------------------------------------------------|-------------|-------------------|---------------------------------------------|
| L-01 | SNAPSHOT version bump → cache hit (NORMALIZED_VERSION)   | F5.7, F12.3 | **P16**           | ❌ `SnapshotVersionBumpCacheHitTest`         |
| L-02 | SNAPSHOT bump + `calculateProjectVersionChecksum` → miss | F12.2       | **P16**           | ❌ `SnapshotVersionBumpWithChecksumFlagTest` |
| L-03 | `adjustMetaInf=true` normalizes MANIFEST.MF              | F12.1       | **P01**           | ❌ `MetaInfVersionAdjustmentTest`            |
| L-04 | CI-friendly `${revision}` version: cache key stable      | F12.4       | **P04**           | ❌ `CIFriendlyRevisionVersionTest`           |

### Group M: Portability & Cross-Environment (P2)

| ID   | Scenario                                                             | Features    | Reference Project | Status                                   |
|------|----------------------------------------------------------------------|-------------|-------------------|------------------------------------------|
| M-01 | Absolute path stripped from effective POM → same key across machines | portability | PARAM P01-P19     | ❌ `AbsolutePathNormalizationTest`        |
| M-02 | Tracked `File` property normalized to relative                       | portability | **P19**           | ❌ `TrackedPropertyPathNormalizationTest` |

### Group N: Project Type Variations (P1/P2)

| ID   | Scenario                                                 | Features | Reference Project              | Status                                 |
|------|----------------------------------------------------------|----------|--------------------------------|----------------------------------------|
| N-01 | WAR packaging: web resources cached/restored             | F6.1     | **P17**                        | ❌ `WarPackagingTest`                   |
| N-02 | POM packaging: no source scan, key from effective POM    | F9, F2   | **P02**                        | ❌ `PomPackagingTest`                   |
| N-03 | Test-jar classifier artifact cached                      | F5.2     | **P06**                        | ❌ `TestJarProjectTest`                 |
| N-04 | Shade plugin: shaded JAR is the cached artifact          | F6.1     | new: `shade-plugin-project`    | ❌ `ShadePluginArtifactReplacementTest` |
| N-05 | Assembly plugin: exclusion pattern prevents ZIP in cache | F6.4     | new: `assembly-plugin-project` | ❌ `AssemblyPluginZipExcludeTest`       |

### Group O: Report Generation & Diagnostics (P2/P3)

| ID   | Scenario                                        | Features | Reference Project | Status                       |
|------|-------------------------------------------------|----------|-------------------|------------------------------|
| O-01 | `build-cache-report.xml` present after build    | F13.5    | PARAM P01-P19     | ❌ `CacheReportGeneratedTest` |
| O-02 | Report shows CACHED / REBUILT status per module | F13.5    | **P02**           | ❌ `CacheReportStatusTest`    |
| O-03 | `buildinfo.xml` FileHash debug entries          | F13.3    | **P01**           | ❌ `BuildInfoXmlDebugTest`    |

### Group P: Internal Architecture (P2/P3)

| ID   | Scenario                                              | Features   | Reference Project | Status                               |
|------|-------------------------------------------------------|------------|-------------------|--------------------------------------|
| P-01 | Stale `target/` artifacts not leaked into cache       | F6.7       | PARAM P01-P19     | ❌ `StagingRemovesStaleClassesTest`   |
| P-02 | Extra CLI goal after lifecycle goal (regression #399) | implicit   | PARAM P01-P19     | ❌ `AdditionalGoalAfterLifecycleTest` |
| P-03 | `Build` round-trip serialization (unit test)          | XmlService | —                 | ❌ `BuildSerializationRoundTripTest`  |
