...
-->
# Maven Test Projects — Generic Configuration Reference

Minimal orthogonal set of Maven project configurations covering every distinct Maven 3 and
Maven 4 project setup. This document is extension-agnostic: the projects are defined purely
in terms of Maven features, with no reference to the build-cache extension or any other
specific extension.

> **Purpose:** Provide a reusable reference test set for validating build cache extension
> works correctly across all flavors of Maven project configurations.
>
> **Design principle:** Each configuration adds exactly one new Maven behavior dimension
> not present in any other configuration. No two configurations duplicate each other's unique
> dimension.
>
> **Count: 18 configurations** (P01–P18). Extension-specific scenarios are tracked separately
> in `maven-test-projects-ext-specific.md`.

---

## Design Debate: 14-config vs 19-config approach

### What the alternative proposal (14 configs) gets right

| Alternative strength                                                       | Verdict                                                                 | Action taken              |
|----------------------------------------------------------------------------|-------------------------------------------------------------------------|---------------------------|
| Each config adds **exactly one** new unique behavior                       | ✅ Accept — superior orthogonality principle                             | Adopted as governing rule |
| `P03_remote_parent_no_relativePath` — isolated config                      | ✅ Accept — incorrectly merged into a larger config previously           | Restored as **P03**       |
| `P07_repository_mirror_snapshot_behavior` — dedicated repos/mirrors config | ✅ Accept — was buried in an overloaded "settings-driven" config         | Restored as **P09**       |
| `P11_toolchains_multi_jdk` — isolated toolchains config                    | ✅ Accept — toolchain failures need isolation from profile failures      | Restored as **P13**       |
| `P12_distribution_management_and_deploy` — dedicated distrib config        | ✅ Accept — important K-dimension, was bundled in parent-chain           | Restored as **P14**       |
| `P13_artifact_relocation` — isolated relocation config                     | ✅ Accept — relocation is a consumer-side behavior, separate from deploy | Restored as **P15**       |
| Nested multi-module for reactor advanced tests                             | ✅ Accept — `-pl -am -rf` tests are cleaner on a proper dependency graph | Adopted in **P10**        |

### What the current approach correctly defends

| Current strength                       | Alternative gap                                                                 | Why it matters                                                                                                                                 |
|----------------------------------------|---------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------|
| **Maven 4 native features**            | Entirely absent from alternative (`maven_version: "3.x+"`)                      | Consumer POM split, `<packaging>bom</packaging>`, `<subprojects>` are production-ready Maven 4 behaviors                                       |
| **Parallel reactor as isolated test**  | Alternative bundles `-T`, `-pl`, `-am`, `-rf`, SNAPSHOT all in one config (P08) | Thread-safety failures are hard to attribute when mixed with partial-build and snapshot logic; dedicated `-T N` test isolates concurrency bugs |
| **WAR packaging**                      | Alternative has no WAR config                                                   | WAR lifecycle has distinct default bindings; WAR files require separate verification                                                           |
| **Core extension + forked invocation** | Alternative's P09 is underspecified                                             | A child Maven process started by `maven-invoker-plugin` has its own independent session; this is a distinct Maven behavior                     |

### Synthesis decision

The alternative's **orthogonality discipline** is the key insight to import. The original
matrix suffered from "behavior stacking" — single configs covering 4–5 independent dimensions.
The merged result restores the configs the alternative correctly identified as needing isolation,
while retaining Maven 4 and WAR packaging configs the alternative omits.

---

## 1. Test Feature Dimensions
...
## 2. Orthogonal Test Configuration Matrix

> **Legend**
> - **Structure:** `1` single · `MM-flat` flat multi-module · `MM-nest` nested hierarchical
> - **Parent:** `super` Super POM only · `local` reactor · `remote` external artifact · `chain`
> - **Dep Mgmt:** `inline` · `inherit` · `bom1` single BOM · `multi-bom` · `mixed`
> - **Plugin Mgmt:** `inline` · `pm` `<pluginManagement>` · `inherit`
> - **Profiles:** `—` · `prop` property · `os` · `file` · `jdk` · `abd` activeByDefault · `settings` · `cli`
> - **Maven:** `3` · `4` · `3+4`
> - **Unique behavior** = the single Maven dimension this config adds that no other config exercises
...
### 4.1 Complete Maven behavior checklist
...
### 4.2 Legitimately deferred (not in required checklist)
